import { BREADCRUMBTYPES, BREADCRUMBCATEGORYS } from '../../shared/src/index'
import { logger, validateOption, getTimestamp, slientConsoleScope, _support } from '../../utils/src/index'
import { BreadcrumbPushData, InitOptions } from '../../types/src/index'
...
export class Breadcrumb {
  ...
  immediatePush(data: BreadcrumbPushData): void {
    data.creationDate || (data.creationDate = getTimestamp())
    if (this.stack.length >= this.maxBreadcrumbs) {
      this.shift()
    }
    this.stack.push(data)
    // make sure xhr fetch is behind button click
    this.stack.sort((a, b) => a.creationDate - b.creationDate)
